...
export const AdminDashboardModal: React.FC<AdminDashboardModalProps> = ({ isOpen, onClose }) => {
  ...
  const toggleHashReveal = (userId: string) => {
  ...
  const handleExportCSV = () => {
    const headers = ['User ID', 'Full Name', 'Email', 'Phone Number', 'Password Hash (SHA-256)', 'Password Masked', 'Registration Date', 'Status', 'Wallet Balance (PHP)', 'Total Deposited (PHP)', 'Active Nodes', 'Role'];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/90 backdrop-blur-md overflow-hidden">
      ...
      >
        ...
        {/* Enterprise Compliance Notice Banner */}
        <div className="bg-cyan-950/40 border-b border-cyan-900/40 px-4 sm:px-6 py-2.5 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2 text-cyan-300">
            ...
            <span className="leading-tight">
              <strong>Enterprise Security & Standards Screening:</strong> Passwords are cryptographically transformed into salted SHA-256 digests in strict compliance with OWASP, ISO 27001, and enterprise data privacy standards. Plaintext credentials are never stored unencrypted.
            </span>
          </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 bg-slate-950 text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
                      ...
                      <th className="py-3 px-4">Password Security Record</th>
                  ...
                  <tbody className="divide-y divide-slate-800/60 font-sans">
                    {filteredUsers.map((user) => {
                      const isRevealed = !!revealedHashes[user.id];
                      return (
                        <tr
                          key={user.id}
                        ...
                        >
                          ...
                          {/* Password Security Record (Enterprise Compliant) */}
                          <td className="py-3 px-4">
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5 font-mono text-cyan-300">
                                ...
                                <button
                                  type="button"
                                  onClick={() => toggleHashReveal(user.id)}
                                  className="text-slate-500 hover:text-cyan-300 p-0.5 rounded"
                                  title={isRevealed ? 'Hide hash digest' : 'Inspect cryptographic SHA-256 digest'}
                                >
                                  {isRevealed ? (
                              ...
                              {isRevealed ? (
                                <div className="bg-slate-950 p-1.5 rounded border border-cyan-900/60 font-mono text-[9px] text-cyan-400 flex items-center justify-between gap-1 max-w-[210px]">
                                  <span className="truncate" title={user.passwordHash}>
                                    sha256:{user.passwordHash.slice(0, 14)}...{user.passwordHash.slice(-6)}
                                  </span>
                                  <button
                                    onClick={() => copyText(user.passwordHash, 'Hash', `hash-${user.id}`)}
            ...
            >
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3 text-xs">
                ...
                <div className="flex justify-between items-center py-1 border-b border-slate-800/80">
                  ...
                  <span className="font-mono text-cyan-300 tracking-wider font-bold">
                    {inspectUser.passwordMasked}
                  </span>
                </div>
                <div className="py-1 border-b border-slate-800/80 space-y-1">
                  <div className="flex justify-between text-slate-400">
                    ...
                    <button
                      onClick={() => copyText(inspectUser.passwordHash, 'Hash', 'inspect-hash')}
                  ...
                  <div className="bg-slate-900 p-2 rounded-xl border border-slate-800 font-mono text-[10px] text-cyan-300 break-all">
                    {inspectUser.passwordHash}
                  </div>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-800/80">
              ...
              <form onSubmit={handleCreateUser} className="space-y-3 text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-[10px] text-slate-500">
                    Will be stored as salted SHA-256 digest adhering to enterprise security standards.
                  </span>
                </div>
