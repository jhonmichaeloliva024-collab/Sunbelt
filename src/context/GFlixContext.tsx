...
export const GFlixProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const registerUser = async (
    ...
    // Generate cryptographic hash conforming to enterprise security standards
    const passwordHash = await hashPassword(passwordPlain);
    const passwordMasked = maskPassword(passwordPlain);
  ...
  const loginUser = async (
    ...
    const candidateHash = await hashPassword(passwordPlain);

    const user = users.find(
      (u) =>
        (u.email.toLowerCase() === input ||
          (u.username && u.username.toLowerCase() === input) ||
          u.phoneNumber.replace(/\D/g, '') === input.replace(/\D/g, '')) &&
        u.passwordHash === candidateHash
    );
  ...
  const addUserManually = async (userData: {
    ...
    const passwordHash = await hashPassword(userData.passwordPlain);
    const passwordMasked = maskPassword(userData.passwordPlain);
    const newUser: UserAccountRecord = {
